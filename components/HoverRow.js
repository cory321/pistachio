...
* With help from the design/UX folks, we decided on these requirements for table body rows and hover content:
* - Hover content is always and immediately shown for, and only for, the row over which the mouse is located
*     with the following exceptions.
* - After entering the table body, no new hover content in any row is shown for one second,
*     though previous content shown on exit might still be shown per the following requirement.
* - After spending at least one second in the table body then exiting the table body,
...
export default function HoverRow( props ) {
	...
	return (
		<tr onMouseEnter={ mouseEnter } onMouseLeave={ mouseLeave }>
			...
			{ React.cloneElement( props.children[ props.children.length - 1 ], { mouseIsOver: mouseIsOver } ) }
		</tr>
	);
}
