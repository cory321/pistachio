...
export function headersFromMessage( normalized ) {
	...
	if ( normalized.cc ) {
		headers.push(
			`CC     : ${normalized.cc.map( addressFormat ).join( ', ' )}`
		)
	}
	...
	return function candidateFromMessage( message ) {
		...
		const a11ns = recipients.filter( recipient => {
			...
			return (
				...
				&&
				(
					( ~ address.indexOf( '@automattic.com' ) )
					||
					( ~ address.indexOf( '@a8c.com' ) )
				)
			)
		} )

		sources = sources.concat( a11ns.map( a11n => ( {
			name: a11n.name ? `${a11n.name} <${a11n.address}>` : a11n.address,
			id: SOURCE_REFERRAL
		} ) ) )

		// jobs+foo@ addresses
		const plusSources = recipients.map( recipient => {
		...
		const applications = [ {
			...
			source_id: sources.map( source => source.id ).filter( sourceId => sourceId ).shift(),
